...
router.beforeEach(async (to,from,next)=>{
  ...
  if(token){
    ...
    // 用户没登录，点击去往我的订单，search页面和center系列页面就会被拦截下来
        ...
        if(targetPath.indexOf('/trade') !== -1 || targetPath.indexOf('/') !== -1 || targetPath.startsWith('/center')){
          //.startSwitch  返回的是true或false，以/center开头的都被拦截
        next('/login?redirect='+targetPath) //跳转到原来在的页面
        } else{
          next()
        }
